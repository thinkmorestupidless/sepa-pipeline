 ...
 */
abstract class AkkaClusterStreamlet extends Streamlet[AkkaClusterStreamletContext] {
   ...
   */
  override protected final def createContext(config: Config): AkkaClusterStreamletContext = {
    (for {
      ...
      AkkaClusterStreamletContextImpl(updatedConfig)
    })
      .recoverWith {
  ...
  override final def run(context: AkkaClusterStreamletContext): StreamletExecution = {
   ...
   */
  protected def createLogic(): AkkaClusterStreamletLogic
