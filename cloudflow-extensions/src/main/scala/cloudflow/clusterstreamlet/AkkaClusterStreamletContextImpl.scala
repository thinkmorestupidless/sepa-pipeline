...
package cloudflow.clusterstreamlet
...
import cloudflow.akkastream.{ KafkaSinkRef, WritableSinkRef }
...
object AkkaClusterStreamletContextImpl {
  def apply(streamletDefinition: StreamletDefinition): AkkaClusterStreamletContextImpl =
    new AkkaClusterStreamletContextImpl(streamletDefinition)
}

/**
 * Implementation of the StreamletContext trait.
 */
final class AkkaClusterStreamletContextImpl(
    private[cloudflow] override val streamletDefinition: StreamletDefinition
) extends AkkaClusterStreamletContext {

  def clusterNamespaceConf = ConfigFactory.
    parseString("akka.discovery.kubernetes-api.pod-namespace=\"" + streamletDefinition.appId + "\"")

  def clusterConfig = config.getConfig(streamletDefinition.streamletRef)

  implicit val system: ActorSystem = ActorSystem(
    "akka_streamlet",
    clusterNamespaceConf.withFallback(clusterConfig.withFallback(config)))
  ...
  val streamletExecution = new StreamletExecution() {
    ...
    def stop(): Future[Dun] = AkkaClusterStreamletContextImpl.this.stop()
  }
  ...
  override def findSavepointPathForPort(port: StreamletPort): SavepointPath = {
